...
export function StudyAbroadInterviewClient() {
  const [messages, setMessages] = React.useState<ChatMessage[]>([]);
  const [input, setInput] = React.useState("");
  const [loading, setLoading] = React.useState(false);
  const [generating, setGenerating] = React.useState(false);
  const [done, setDone] = React.useState(false);
  const [studyAbroadProfile, setStudyAbroadProfile] = React.useState<
    string | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    async function init() {
      setLoading(true);
      try {
        const res = await fetch("/api/profile/study-abroad-interview", {
          method: "POST",
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      const genRes = await fetch("/api/profile/study-abroad-interview/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const { studyAbroadProfile: profile } = (await genRes.json()) as {
      ...
      const patchRes = await fetch("/api/profile", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ studyAbroadProfile: profile }),
      });
      if (!patchRes.ok) {
        throw new Error("Failed to save study abroad profile");
      }

      setStudyAbroadProfile(profile);
      setDone(true);
      toast.success("Study abroad profile saved!");
    } catch (err) {
      toast.error(
  ...
  const showSave = userExchangeCount >= 4 && !done;

  if (done && studyAbroadProfile) {
    return (
